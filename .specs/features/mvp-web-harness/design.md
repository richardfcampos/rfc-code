...
- **Risco**: ~~portabilidade dos session files entre config dirs não é documentada~~ **RESOLVIDO — spike provou AMBOS portáveis (ver Risks & Concerns)**; transplante+resume é o mecanismo primário viável p/ Claude e Codex.
...
| Portabilidade de session files entre config dirs (handoff HUB-12) | `handoff.service.ts`; Claude `<CLAUDE_CONFIG_DIR>/projects/<enc-cwd>/<id>.jsonl`, Codex `<CODEX_HOME>/sessions/YYYY/MM/DD/rollout-*.jsonl` | Se o CLI não achar a sessão transplantada em outro config dir, o handoff primário (transplante+resume) é inviável e sobra só a degradação | **RESOLVIDO — spike com EXPERIMENTO REAL (2026-07-23; `claude` 2.1.195, `codex-cli` 0.143.0). AMBOS PORTÁVEIS.** Método: config dir efêmero VAZIO (sem credencial, sem turno de API) + controle vs. tratamento (copiar o session file real p/ o novo config dir). **Claude** — comando `cd "/Volumes/External Code/INTEL/Code/personal" && CLAUDE_CONFIG_DIR=<efêmero> claude -p "reply OK" --resume 54bc163a-…`: controle (dir vazio) → `No conversation found with session ID: 54bc163a-…`; tratamento (`.jsonl` copiado p/ `<cfg>/projects/-Volumes-External-Code-INTEL-Code-personal/54bc163a-….jsonl`) → `Not logged in · Please run /login`. A mudança de "not found" p/ "not logged in" prova que a sessão foi ACHADA e CARREGADA; só faltou auth do perfil de destino. Descoberta é `CLAUDE_CONFIG_DIR` + `projects/<cwd-encoded>/<id>.jsonl` (encoding: `/` e espaço → `-`, provado por `-Volumes-External-Code-INTEL-Code-personal` ↔ `/Volumes/External Code/INTEL/Code/personal`); resume exige rodar no mesmo cwd. **Codex** — comando `CODEX_HOME=<efêmero> codex exec resume 019f4303-… "reply OK" --skip-git-repo-check`: controle → `Error: thread/resume failed: no rollout found for thread id 019f4303-…`; tratamento (rollout copiado p/ `<CODEX_HOME>/sessions/2026/07/08/rollout-…-019f4303-….jsonl`) → `401 Unauthorized` em `wss://api.openai.com/v1/responses`. Sumiu o "no rollout found" → thread carregado; parou só na auth. Descoberta varre `<CODEX_HOME>/sessions/**` por id, **cwd-independente**. **Conclusão:** o transplante do session file + resume nativo é o mecanismo PRIMÁRIO viável p/ Claude e Codex; único gate = perfil de DESTINO autenticado (garantido no handoff). Degradação (nova sessão semeada com transcript) fica p/ providers sem session file portável (Cursor/OpenCode, fora do escopo de handoff do MVP). Nenhum turno de API consumido — ambos pararam no passo de auth (Claude nem tentou rede; Codex recebeu 401, sem tokens). |
