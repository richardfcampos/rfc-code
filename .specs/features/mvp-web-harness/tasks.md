...
**Revisão (2026-07-23)**: correção de contexto — `intel` é host **macOS** (não Linux), então a limitação de `network_mode: host` no Docker Desktop descrita acima se aplica em produção, não só em dev. `deploy/Dockerfile` em si não mudou nesta revisão (imagem segue válida); o ajuste de networking está em T16.
...
**Revisão (2026-07-23)**: `intel` é host **macOS** (não Linux) — a limitação de `network_mode: host` no Docker Desktop macOS descrita acima portanto se aplica ao alvo real de produção, invalidando a decisão original de usar host networking. Substituído por bridge padrão + `ports:` publicando apenas na interface tailnet do host (`BIND_IP:PORT:3001`, default `100.70.101.109:7789`); guard `assertTrustedModeBindIsSafe` ganhou contrato `AUTH_TRUSTED_CONTAINER_BIND=1` (server/middleware/auth.js) para aceitar bind não-loopback dentro do container quando o operador garante que o publish do Docker está restrito — comportamento default (sem a env) permanece idêntico, testes de recusa existentes inalterados. `deploy/.env.example` criado (`BIND_IP`/`PORT`, alternativa loopback+`tailscale serve` p/ HTTPS/PWA). Validado nesta máquina (que é o próprio `intel`): `docker compose up -d` + smoke completo = **7 passed, 0 failed**, incluindo os checks de curl do host que antes só funcionavam via `docker exec` — confirmado publish exclusivo na interface `100.70.101.109` (loopback e demais interfaces continuam inalcançáveis). Commits: `feat(auth): container-aware bind contract for trusted mode` — `6020260`; `fix(deploy): bridge networking with tailnet publish for macos hosts` — `dffc89f`. 209 testes/0 falhas (207 + 2 novos no guard); build EXIT 0.
