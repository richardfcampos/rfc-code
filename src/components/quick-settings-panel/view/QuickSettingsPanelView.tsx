...
export default function QuickSettingsPanelView() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed right-0 top-0 z-40 h-full w-64 transform border-l border-border bg-card shadow-[var(--shadow-float)] transition-transform duration-150 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} ${isMobile ? 'h-screen' : ''}`}
      >
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-30 bg-[var(--scrim)] backdrop-blur-sm transition-opacity duration-150 ease-out"
